...
MetaWidget *render_bool(AbstractAttribute &abstract_attr,
                        const RowContext &,
                        QWidget *parent)
{
  auto             &attr = static_cast<Attribute<bool> &>(abstract_attr);
...
{
  const std::type_index type = std::type_index(typeid(bool));
  registry.add(kDesignName, type, "Toggle", render_bool);
  registry.add(kDesignName, type, "Checkbox", render_bool);
  registry.add(kDesignName, type, "BinaryButtons", render_bool);
  registry.add(kDesignName, type, kAnyWidgetType, render_bool);
}
